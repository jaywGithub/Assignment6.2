package com.example.jayson.assignment6.domain.courierCost.settings;

import java.io.Serializable;

/**
 * Created by JAYSON on 2016-04-22.
 */
public class TransportCostType implements Serializable {

    private Long id;
    private String transportType;
    private double cost;
    private String state;

    public TransportCostType(Builder builder)
    {
        this.id = builder.id;
        this.transportType = builder.transportType;
        this.cost = builder.cost;
        this.state = builder.state;
    }

    public Long getId()
    {
        return id;
    }

    public String getTransportType()
    {
        return transportType;
    }

    public double getCost()
    {
        if(transportType.equalsIgnoreCase("Regular"))
        {
            cost = 50.0;
        }

        else if(transportType.equalsIgnoreCase("Express1"))
        {
            cost = 100.0;
        }

        else if(transportType.equalsIgnoreCase("Express2"))
        {
            cost = 150.0;
        }

        return cost;
    }

    public String getState()
    {
        return state;
    }

    public static class Builder
    {
        private Long id;
        private String transportType;
        private double cost;
        private String state;

        public Builder id(Long value)
        {
            this.id = value;
            return this;
        }

        public Builder transportType(String value)
        {
            this.transportType = value;
            return this;
        }

        public Builder cost(double value)
        {
            this.cost = value;
            return this;
        }

        public Builder state(String value)
        {
            this.state = value;
            return this;
        }

        public Builder copy(TransportCostType value)
        {
            this.id = value.id;
            this.transportType = value.transportType;
            this.cost = value.cost;
            this.state = value.state;

            return this;
        }

        public TransportCostType build()
        {
            return new TransportCostType(this);
        }
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;

        TransportCostType transportCostType = (TransportCostType) o;

        return id != null ? id.equals(transportCostType.id) : transportCostType.id == null;

    }

    @Override
    public int hashCode() {
        return id != null ? id.hashCode() : 0;
    }
}
